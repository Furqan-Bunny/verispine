...
/**
 * Set tracking on an order by hand.
 *
 * For shipments arranged outside the platform — a seller who dropped the item at
 * a counter, or a freight carrier that issued its own pro number. Recording it
 * here is what lets the buyer see anything at all, so it deliberately does NOT
 * require the order to already be shipped.
 */
router.put('/orders/:orderId/tracking', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { orderId } = req.params;
    ...
    const carrier = String(shippingCarrier || 'USPS').trim();

    await orderRef.update({
      trackingNumber: tracking,
      shippingCarrier: carrier,
      shippingStatus: order.shippingStatus || 'shipped',
      // Only stamp shippedAt the first time, so re-entering a corrected tracking
      // number doesn't reset the shipping date shown to the buyer.
      ...(order.shippedAt ? {} : { shippedAt: timestamp }),
      updatedAt: timestamp,
      updatedBy: req.user.uid
    });

    // Mirror onto the shipment record if one exists, so the tracking page and the
    // order page can't disagree about the number.
    try {
      const shipmentRef = db.collection('shipments').doc(orderId);
      if ((await shipmentRef.get()).exists) {
        await shipmentRef.update({ trackingNumber: tracking, carrier, updatedAt: timestamp });
      }
    } catch (e) {
      console.warn('Tracking update: shipment mirror failed:', e.message);
    }

    // Tell the buyer. Non-fatal — the tracking is saved either way.
    try {
      if (order.buyerId) {
        const buyerDoc = await db.collection('users').doc(order.buyerId).get();
        if (buyerDoc.exists) {
          await emailService.sendOrderShipped(buyerDoc.data(), { ...order, orderId, shippingCarrier: carrier }, tracking);
        }
      }
    } catch (e) {
    ...
    res.json({ success: true, message: 'Tracking information saved', data: { orderId, trackingNumber: tracking, shippingCarrier: carrier } });
  } catch (error) {
...
/**
 * Internal admin notes on an order. Never surfaced to the buyer or seller —
 * these are for the operations team, so the field is written but not returned by
 * any customer-facing endpoint.
 */
router.put('/orders/:orderId/notes', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    await orderRef.update({
      ...
      adminNotesUpdatedBy: req.user.uid,
      updatedAt: timestamp
    });

    res.json({ success: true, message: 'Admin notes saved', data: { orderId } });
  } catch (error) {
